...
impl Phase {
    pub fn message(self) -> &'static str {
        match self {
            ...
            Self::Fetching => "Fetching repositories...",
            Self::Refreshing => "Refreshing repositories...",
        }
    }
}
...
impl EventSink for Sender<Event> {
    fn emit(&self, event: Event) -> Result<(), AppError> {
        self.send(event).map_err(|_| AppError::internal("refresh event receiver disconnected"))
    }
}
