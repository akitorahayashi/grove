...
#[cfg(unix)]
#[test]
fn add_does_not_reuse_a_group_when_only_its_repository_is_a_symlink() {
    use std::os::unix::fs::symlink;

    let ctx = TestContext::new();
    let remote = ctx.create_remote("repository-alias");
    let existing = initialize_repository(&ctx, "Actual/existing", &remote.url());
    initialize_repository(&ctx, "Actual/kebab", &remote.url());
    fs::create_dir(ctx.workspace().join("Alias")).unwrap();
    symlink(existing, ctx.workspace().join("Alias/existing")).unwrap();
    ctx.write_config(&format!("version = 2\n\n[groups.Alias]\nexisting = \"{}\"\n", remote.url()));

    ctx.cli()
        .args(["add", "Actual/kebab"])
        .assert()
        .success()
        .stderr(predicate::str::contains("+ kebab Actual/kebab"));

    let contents = fs::read_to_string(ctx.config_path()).unwrap();
    assert!(contents.contains("[groups.Alias]"));
    assert!(contents.contains("[groups.Actual]"));
}

#[cfg(unix)]
#[test]
fn add_reuses_an_empty_configured_group_for_the_same_directory() {
    use std::os::unix::fs::symlink;

    let ctx = TestContext::new();
    let remote = ctx.create_remote("empty-group-alias");
    initialize_repository(&ctx, "Actual/kebab", &remote.url());
    symlink(ctx.workspace().join("Actual"), ctx.workspace().join("Configured")).unwrap();
    ctx.write_config("version = 2\n\n[groups.Configured]\n");
