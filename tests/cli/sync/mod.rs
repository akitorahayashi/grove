...
fn sync_updates_default_branch_and_restores_current_branch() {
    ...
    assert_eq!(current_branch(&ctx.workspace().join("frontend")), "feature/login");
}
...
fn sync_reports_blocked_repositories_and_exits_with_failure() {
    ...
    let mismatched_config = ctx.write_single_repository_config("blog", &replacement.url(), None);
...
fn sync_dry_run_redacts_credentials_and_secret_query_values() {
    ...
    let config = ctx.write_single_repository_config(
        "blog",
        "https://user:credential@example.com/repo.git?access_token=secret-value&branch=main",
        None,
    );
...
fn sync_redacts_url_echoed_by_clone_failure() {
    ...
    let path = path_with_wrapper(
        &ctx,
        "sync-clone",
        "if [ \"$1\" = clone ]; then echo 'fatal: clone failed' >&2; exit 1; fi",
    );
    let config = ctx.write_single_repository_config(
        "blog",
        "https://user:credential@example.com/repo.git?password=secret-value",
        None,
    );
...
"#,
    ...
    let config = ctx.write_single_repository_config(
        "blog",
        "https://user:credential@example.com/repo.git?api_key=secret-value",
        None,
    );
...
fn sync_blocks_non_repository_missing_origin_and_detached_head() {
    ...
    let config =
        non_repository.write_single_repository_config("blog", "git@example.com:blog.git", None);
    non_repository
        .cli()
        .arg("--config")
        .arg(config)
    ...
    let config = missing_origin.write_single_repository_config("blog", &remote.url(), None);
    ...
    let config = detached.write_single_repository_config("blog", &remote.url(), None);
...
fn sync_declines_a_mismatched_repository_without_running_its_fsmonitor_hook() {
    ...
    let ctx = TestContext::new();
    let remote = ctx.create_remote("blog");
    let matching_config = ctx.write_single_repository_config("blog", &remote.url(), None);
...
fn sync_blocks_missing_default_and_configured_branches() {
    ...
    let config = missing_default.write_single_repository_config("blog", &remote.url(), None);
    ...
    let initial = missing_local.write_single_repository_config("blog", &remote.url(), None);
    missing_local.cli().arg("--config").arg(&initial).arg("sync").assert().success();
    let config = missing_local.write_single_repository_config("blog", &remote.url(), Some("ghost"));
    missing_local
        .cli()
        .arg("--config")
        .arg(&config)
    ...
    let initial = missing_remote.write_single_repository_config("blog", &remote.url(), None);
    missing_remote.cli().arg("--config").arg(&initial).arg("sync").assert().success();
    run_git(&missing_remote.workspace().join("blog"), &["branch", "ghost"]);
    let configured =
        missing_remote.write_single_repository_config("blog", &remote.url(), Some("ghost"));
    missing_remote
        .cli()
        .arg("--config")
        .arg(configured)
...
fn sync_blocks_ahead_and_diverged_default_branches() {
    ...
    let config = ahead.write_single_repository_config("blog", &remote.url(), None);
    ...
    let config = diverged.write_single_repository_config("blog", &remote.url(), None);
...
fn sync_reports_fetch_and_clone_failures() {
    ...
    let initial = fetch_failure.write_single_repository_config("blog", &remote.url(), None);
...
fn configured_default_branch_overrides_stale_origin_head() {
    ...
    remote.add_commit("remote.txt", "remote\n");
    let configured = ctx.write_single_repository_config("blog", &remote.url(), Some("main"));
